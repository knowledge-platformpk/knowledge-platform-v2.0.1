                        ...
                        if st.button(f'Save GeoTIFF'): #if st.button(f'Save {selected_index}'):
                            ...
                            try:
                                if selected_index_uploaded == 'SRTM':
                                    ...
                                    download_url = st.session_state['temp_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Enhanced Vegetation Index (EVI)':
                                    download_url = st.session_state['evi_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Evapotranspiration ET':
                                    download_url = st.session_state['et_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Keetch-Byram Drought Index':
                                    download_url = st.session_state['kbdi_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Land Cover (2020)':
                                    download_url = st.session_state['landcover_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Land Surface Temperature':
                                    download_url = st.session_state['lst_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Latent Heat Flux (LE)':
                                    download_url = st.session_state['lhf_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Leaf Area Index':
                                    download_url = st.session_state['LAI_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Population':
                                    download_url = st.session_state['population_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Potential Evapotranspiration (PET)':
                                    download_url = st.session_state['pet_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Precipitation':
                                    download_url = st.session_state['Precp_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Snow Cover':
                                    download_url = st.session_state['snowcover_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Soil Moisture':
                                    download_url = st.session_state['soil_moisture_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Soil Temperature':
                                    download_url = st.session_state['soil_temperature_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Transpiration (TP)':
                                    download_url = st.session_state['tp_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
                                elif selected_index_uploaded == 'Wind Speed':
                                    download_url = st.session_state['windspeed_image'].getDownloadURL({'scale': 90, 'dimensions': [2000,2000]})
