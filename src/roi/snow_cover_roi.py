...
def get_snow_cover_image_roi(json_data, from_date, to_date):
    ...
    mean_snow_cover_image = snow_cover_collection.mean().clip(roi)
...
def create_snow_cover_timeseries_roi(json_data, from_date, to_date):
    roi = ee.FeatureCollection(json_data)
    ...
    # Filter the ERA temperature collection by date
    snow_cover_collection = ee.ImageCollection("MODIS/061/MOD10A1").filterBounds(roi).filterDate(start_date, end_date).select('NDSI_Snow_Cover').map(scale_index)
